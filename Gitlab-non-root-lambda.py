...
from kubernetes.client import ApiClient, ApiException, V1NodeSelectorRequirement, V1NodeSelector, V1NodeSelectorTerm, V1Affinity, V1NodeAffinity
...
# EKS Job Launcher
# --------------------
def trigger_eks_job(cluster_name, region, job_name, image, env_vars, args_list, namespace=NAMESPACE):
    """
    ...
    resources = client.V1ResourceRequirements(
        requests={
            "cpu": os.environ.get("K8_RESOURCES_REQUEST_CPU", "800m"),
            "memory": os.environ.get("K8_RESOURCES_REQUEST_MEMORY", "1Gi"),
        },
        limits={
            "memory": os.environ.get("K8_RESOURCES_LIMIT_MEMORY", "5Gi"),
        }
    )
    ...
    node_selector_requirement = V1NodeSelectorRequirement(
        key="workload",
        operator="In",
        values=["jenkins"]
    )
    node_selector_term = V1NodeSelectorTerm(
        match_expressions=[node_selector_requirement]
    )
    ...
    template = client.V1PodTemplateSpec(
        metadata=client.V1ObjectMeta(name=job_name, labels=pod_labels), spec=pod_spec
    )
...
# --------------------
def check_job_status(cluster_name, region, job_name, namespace=NAMESPACE):
    """
    ...
    try:
        ...
        # Use conditions (K8s-recommended) for terminal state. Only when type=Failed/Complete
        # and status=True is the job permanently done. Otherwise failed>0 with active>0 means
        # a retry is running (backoff_limit=1 allows 1 retry).
        conditions = getattr(job.status, "conditions", None) if job.status else None
        if conditions:
            for c in conditions:
                if getattr(c, "type", None) == "Complete" and getattr(c, "status", "") == "True":
                    status = "SUCCEEDED"
                    break
                if getattr(c, "type", None) == "Failed" and getattr(c, "status", "") == "True":
                    status = "FAILED"
                    break
        if status == "SCHEDULED":
            if job.status.succeeded:
                ...
                # Fallback: FAILED only when failed > backoffLimit (retries exhausted) and no active pods.
                # With backoff_limit=1: failed=1 + active=0 is the retry window (don't mark FAILED yet).
                backoff_limit = getattr(job.spec, "backoff_limit", 6) if job.spec else 6
                failed = getattr(job.status, "failed", 0) or 0
                active = getattr(job.status, "active", 0) or 0
                if failed > backoff_limit and active == 0:
                    status = "FAILED"
        ...
        if status != "SCHEDULED" or (job.status.active and job.status.active > 0):
            ...
            if pod_list.items:
                ...
                try:
                    # Limit to last 500 lines to avoid Lambda timeout/OOM for long-running jobs
                    logs = core_v1.read_namespaced_pod_log(
                        name=pod_name, namespace=namespace, container="scanner", tail_lines=500
                    )
                    if status == "FAILED":
...
# EKS Job Deleter
# --------------------
def delete_eks_job(cluster_name, region, job_name, namespace=NAMESPACE):
    """
    Deletes a Kubernetes job (e.g. to allow retry after failure).
    """
    print(f"[INFO] Deleting job: {job_name}")
    try:
        eks = boto3.client('eks', region_name=region)
        cluster_info = eks.describe_cluster(name=cluster_name)['cluster']
        ...
        if e.status == 404:
            print(f"[INFO] Job '{job_name}' not found (already deleted), continuing")
        else:
            print(f"[ERROR] Failed to delete job '{job_name}': {e}")
            raise e
...
def lambda_handler(event, context):
    ...
    try:
        if action == "list_repos":
            ...
            job_name = f"cdefense-hybrid-{job_id}"
            ...
            # Idempotent: check if job already exists before creating (avoids 409 on retry)
            existing = check_job_status(cluster_name, region, job_name)
            if existing["status"] == "SCHEDULED":
                print(f"[INFO] Job '{job_name}' already running, skipping create")
                return {"statusCode": 202, "body": json.dumps(existing)}
            if existing["status"] == "SUCCEEDED":
                print(f"[INFO] Job '{job_name}' already completed successfully, skipping create")
                return {"statusCode": 202, "body": json.dumps(existing)}
            if existing["status"] == "FAILED":
                print(f"[INFO] Job '{job_name}' previously failed, deleting and recreating for retry")
                delete_eks_job(cluster_name, region, job_name)
            ...
            if job_type == "public":
                ...
                job_name_to_check = f"cdefense-hybrid-{job_id}"  # Keep the original spelling for backward compatibility
